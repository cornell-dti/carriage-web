...
export const RidersProvider = ({ children }: RidersProviderProps) => {
  ...
  const updateRiderActive = useCallback(async (riderId: string, active: boolean) => {
    try {
      // Optimistic update
  ...
  const updateRiderInfo = useCallback(async (riderId: string, updates: Partial<Rider>) => {
  ...
  const deleteRider = useCallback(async (riderId: string) => {
    const originalRiders = [...riders];
    try {
      // Optimistic update
