...
export const EmployeesProvider = ({ children }: EmployeesProviderProps) => {
  ...
  const updateDriverInfo = useCallback(async (driverId: string, updates: Partial<Driver>) => {
    ...
    try {
      ...
      setDrivers(prevDrivers =>
        prevDrivers.map(driver =>
          driver.id === driverId ? serverDriver : driver
        ).sort(sortByName)
      );
    } catch (error) {
      // Rollback on error
  ...
  const updateAdminInfo = useCallback(async (adminId: string, updates: Partial<Admin>) => {
    ...
    try {
      ...
      setAdmins(prevAdmins =>
        prevAdmins.map(admin =>
          admin.id === adminId ? serverAdmin : admin
        ).sort(sortByName)
      );
    } catch (error) {
      // Rollback on error
  ...
  const createAdmin = useCallback(async (admin: Omit<Admin, 'id'>) => {
    ...
    try {
      ...
      setAdmins(prevAdmins =>
        prevAdmins.map(a => a.id === tempId ? serverAdmin : a).sort(sortByName)
      );
    } catch (error) {
      // Rollback on error
  ...
  const getAdminById = useCallback((adminId: string): Admin | undefined => {
