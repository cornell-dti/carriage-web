  ...
  return (
    ...
    >
      <CardContent className="p-6">
        <Typography
          variant="h6"
          className="text-lg font-semibold text-[#333] mb-4"
        >
        ...
        <Grid container spacing={2} className="gap-4">
          {statistics.map((stat, index) => (
            <Grid item xs={12} sm={6} key={index}>
              <Card
                className="bg-white rounded-xl transition-all duration-200 cursor-default hover:-translate-y-0.5 hover:shadow-[0_4px_20px_rgba(0,0,0,0.12)]"
                elevation={1}
              >
                <CardContent className="p-5 flex flex-row items-center gap-4 max-md:p-4">
                  ...
                  <Box className="flex flex-col gap-1 grow">
                    {stat.value >= 0 ? (
                      <Typography
                        variant="h4"
                        className="text-[1.75rem] font-bold m-0 text-[#1a1a1a] leading-tight max-md:text-2xl"
                      >
                        {stat.value}
                        {stat.unit && (
                          <Typography
                    ...
                    <Typography
                      variant="body2"
                      className="text-sm text-[#6b7280] m-0 uppercase font-medium tracking-wide"
                    >
                      {stat.label}
                    </Typography>
                  </Box>
