  ...
  return (
    <div className="h-fit max-h-50 bg-white rounded-lg shadow-[0_2px_4px_rgba(0,0,0,0.1)] p-4 text-black flex flex-col">
      <div className="flex items-center mb-2 pb-2 border-b border-[#f0f0f0]">
        ...
        <button
          className="bg-transparent border-0 cursor-pointer p-1 rounded transition-colors duration-200"
          onClick={onAddNew}
        >
      ...
      <div className="max-h-37.5 flex flex-col gap-4 overflow-y-auto pr-2.5">
        ...
        {favorites.map((fav) => (
          <div
            key={fav.id}
            className="flex items-center justify-between bg-[#f9f9f9] rounded-md p-3"
          >
            <div className="flex flex-col gap-1 flex-1">
              <h3 className="m-0 text-[0.95rem] text-[#333] font-semibold mb-1">
                {fav.name}
              </h3>
