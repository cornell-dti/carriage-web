...
import { useOptimisticUpdate, OptimisticUpdateOptions } from './useOptimisticUpdate';
import axios from '../util/axios';

export interface RiderOperations {
  updateRiderActive: (riderId: string, active: boolean) => Promise<string>;
  updateRiderInfo: (riderId: string, updates: Partial<Rider>) => Promise<string>;
...
export function useOptimisticRiders(initialRiders: Rider[]) {
  ...
  const findRiderIndex = useCallback((riderId: string) => {
    return optimisticState.data.findIndex(rider => rider.id === riderId);
  }, [optimisticState.data]);

  const updateRiderActive = useCallback(async (
    riderId: string,
    active: boolean,
    ...
    try {
      ...
      optimisticState.rollbackOperation(
        operationId,
        error instanceof Error ? error : new Error('Failed to update rider status'),
        options
      );
      throw error;
    }
  ...
  const updateRiderInfo = useCallback(async (
    riderId: string,
    ...
    try {
      ...
      optimisticState.rollbackOperation(
        operationId,
        error instanceof Error ? error : new Error('Failed to update rider info'),
        options
      );
      throw error;
    }
  ...
  const createRider = useCallback(async (
    rider: Omit<Rider, 'id'>,
    options: OptimisticUpdateOptions<Rider[]> = {}
  ): Promise<string> => {
  ...
  const deleteRider = useCallback(async (
    riderId: string,
    options: OptimisticUpdateOptions<Rider[]> = {}
  ): Promise<string> => {
    const riderIndex = findRiderIndex(riderId);
    ...
    const updatedRiders = optimisticState.data.filter(rider => rider.id !== riderId);
  ...
  const getRiderById = useCallback((riderId: string): Rider | undefined => {
