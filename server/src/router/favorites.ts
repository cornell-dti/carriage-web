...
router.post('/', validateUser('User'), async (req, res) => {
    ...
    if (!ride) {
        return res.status(404).send({ err: 'Ride not found, unable to favorite a ride that does not exist.' });
    }
...
router.get('/', validateUser('User'), (req, res) => {
    ...
    db.query(res, Favorite, condition, 'userId-index', (favorites: typeof Favorite[]) => {
